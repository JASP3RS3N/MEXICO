...
# Products (menú)
# ---------------------------------------------------------------------------
async def _product_cost(recipe) -> float:
    """Estimate a product's ingredient cost from its recipe (best-effort)."""
    total = 0.0
    for item in recipe or []:
        mat = await db.materials.find_one({"id": item["material_id"]}, {"_id": 0})
        if mat:
...
async def update_product(product_id: str, payload: ProductUpdate, user: dict = Depends(require_owner)):
    ...
    if "recipe" in data and data["recipe"] is not None:
        ...
        updates["cost"] = await _product_cost(recipe)
...
async def update_price(product_id: str, payload: PriceUpdate, user: dict = Depends(require_owner)):
    ...
    res = await db.products.update_one(
        {"id": product_id}, {"$set": {"price": round(float(payload.price), 2)}}
    )
